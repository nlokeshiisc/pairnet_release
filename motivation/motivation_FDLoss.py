...
x_0 = np.arange(-0.25, 0.125, 0.375/n0)
x_1 = np.arange(-0.125, 0.25, 0.375/n1)
x_test = np.concatenate([x_0, x_1])

f0_tst = sample_gaussian_process(x_values=x_test, width=mu_0) + 0.2 # Adding constant for better looking figure
f0_star = f0_tst[:n0]
ftau_tst = sample_gaussian_process(x_values=x_test, width=mu_tau)
f1_tst = f0_tst + ftau_tst + 1.9
f1_star = f1_tst[n0:]
print(x_0)
print(x_1)
print(ftau_tst)

# Plot x_tst, f0_tst, ftau_tst, f1_tst
...
for method in ["nbrs"]: #, "ot"]:
    ...
    for arbitrary_fn in [True]: #, False]:
        ...
        np.random.seed(3)
        for mu_0_hat in [15]: #, 1.5]:
            for mu_tau_hat in [5]: #, 0.25]:
                for ns in [1]: #, 2, 3, 4, 5, 7, 8, 9, 10]:
                    ...
                    plt.clf()
                    plt.cla()
                    # plt.plot(x_test[argsort], f0_tst[argsort], label=r'$\mu_0^{\star}$', linewidth=2)
                    # plt.plot(x_test[argsort], f1_tst[argsort], label=r'$\mu_1^\star$', linewidth=2)
                    # plt.plot(x_test[argsort], ftau_tst[argsort], label=r'$\tau^{\star} = \mu_1^\star - \mu_0^\star$', linewidth=2)
                    plt.plot(x_test[argsort], f0_hat[argsort], label=r'$\widehat{\mu}(\bullet, 0)$', linewidth=5, color="cyan")
                    plt.plot(x_test[argsort], f1_hat[argsort], label=r'$\widehat{\mu}(\bullet, 1)$', linewidth=5, color="limegreen")
                    # plt.plot(x_test[argsort], ftau_tst[argsort], label="true_cate")

                    # Reduce the marker opacity
                    plot_n0 = np.arange(n0)
                    plot_n1 = np.arange(n0, n)
                    plt.scatter(x_test[plot_n0], f0_tst[plot_n0], marker="*", s=180, color="blue", label="Obs Data for T=0", zorder=10)
                    plt.scatter(x_test[plot_n1], f1_tst[plot_n1], marker="p", s=180, color="green", label="Obs Data for T=1", zorder=10)
                    
                    # Number the scatter points
                    for idx in plot_n0:
                        plt.text(x_test[idx]+0.01, f0_tst[idx]-0.15, idx, fontsize=18, color="blue", zorder=12)
                    for idx in plot_n1:
                        plt.text(x_test[idx]+0.01, f1_tst[idx], idx, fontsize=18, color="green", zorder=12)

                    # Plotting the factual error
                    for idx in plot_n0:
                        if idx == 0:
                            plt.plot([x_test[idx], x_test[idx]], [f0_hat[idx], f0_tst[idx]], color="red", linestyle="--", linewidth=2, label="FLoss")
                        else:
                            plt.plot([x_test[idx], x_test[idx]], [f0_hat[idx], f0_tst[idx]], color="red", linestyle="--", linewidth=2)
                    for idx in plot_n1:
                        plt.plot([x_test[idx], x_test[idx]], [f1_hat[idx], f1_tst[idx]], color="red", linestyle="--", linewidth=2)

                    # Plot lines between the matched points
                    for idx in plot_n0:
                        nbr = plot_n1[nbrs_0[idx]]
                        if idx == 0:
                            plt.plot([x_test[idx], x_test[nbr]], [f0_tst[idx], f1_tst[nbr]], color="black", linestyle="-", linewidth=2, label="Pairs")
                        else:
                            plt.plot([x_test[idx], x_test[nbr]], [f0_tst[idx], f1_tst[nbr]], color="black", linestyle="-", linewidth=2)
                    for _, idx in enumerate(plot_n1):
                        nbr = plot_n0[nbrs_1[_]]
                        plt.plot([x_test[idx], x_test[nbr]], [f1_tst[idx], f0_tst[nbr]], color="black", linestyle="-", linewidth=2)

                    # set x label
                    plt.xlabel(r'$x \rightarrow$', fontsize=20)
                    plt.ylabel(r'$Y(t) \rightarrow$', fontsize=20)

                    # Set legend at lower right
                    plt.legend(loc="lower right", fontsize=12)
                    # plt.show()
                    plt.tight_layout()
                    plt.savefig(f"motivation/mot_plot.pdf", bbox_inches='tight', dpi=300)
